...
def main():
    ...
    for raw_dataset in datasets:
        if raw_dataset.equals(pmigma):
            ...
            ref = 'openpose_output'
        standardized_dataset = standardize_joint_number(raw_dataset, ref)
        zerofiltered_dataset = remove_zeroes(standardized_dataset, ref)
        CIfiltered_dataset   = remove_low_CIs(zerofiltered_dataset)
        interpolated_dataset = interpolate_nan_values(CIfiltered_dataset)
        rescaled_dataset     = rescale(interpolated_dataset, ref)
        pivoted_dataset      = pivot(rescaled_dataset)
...
def smooth(dataset, window_size):
    """
